...
import { Clock, CheckCircle, XCircle, ArrowLeft, Pencil, Trash2, Wallet, MessageSquare, Send, TrendingUp, Share2, Mail } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between mb-6">
        <Link href="/markets" className="inline-flex items-center gap-1 text-sm text-zinc-400 hover:text-white">
          <ArrowLeft className="w-4 h-4" /> Back to markets
        </Link>
        <button
          onClick={() => {
            const subject = encodeURIComponent(`Check out this market: ${market.title}`);
            const body = encodeURIComponent(
              `Hey,\n\nI found this prediction market on SaskPoly:\n\n"${market.title}"\n\n${market.description?.slice(0, 200)}...\n\nCheck it out: ${window.location.href}\n\nWhat's your prediction?`
            );
            window.open(`mailto:?subject=${subject}&body=${body}`, "_blank");
          }}
          className="inline-flex items-center gap-1.5 text-sm text-zinc-400 hover:text-emerald-400 transition"
